...
8. createroomactivity에서 입력필드 중 하나라도 입력되지 않으면 완료버튼 비활성화

9. 검색창에 커서 활성화(포커스)되면 검색어를 입력하세요 힌트 비활성화

10. ChatFragment에서 나가기 기능 구현(해당 유저의 UI에서만 삭제. 다시 채팅이 오거나 갈 경우 다시 보이게)

11. 경매 종료시에, 판매자가 판매내역에서 AuctionRoomActivity를 들어갔을 때 detail 하단에 최고 입찰자 3명과 채팅 할 수 있는 기능 구현(gone)

< 할 일 >

1. HomeFragement 우측 상단에 알림 기능 구현( 내 판매내역에 입찰 들어왔을때, 내 입찰이 갱신되었을때(다른사람한테), 내 찜목록이 1시간이하로 남았을때, 
채팅이 왔을때, 내 구매희망 글에 댓글이 달렸을 때 등등)
