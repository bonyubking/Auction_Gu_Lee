...
3. AuctionRoomActivity --> 참가자 수 삭제, 최고가, 입찰가 한글 삭제 , 입찰이 없을때 --> 입찰 없음 표시

4. 판매자가 자신의 AuctionRoomActivity에 들어갔을떄 현재는 bid, chat 버튼이 비활성화 되어있음 --> 안보이게 수정

5. 내가 만든 경매에 입찰이 없을 경우, 판매자가 해당 경매 삭제 할수 기능 추가(실수로 생성했을수 있음)

7. 안읽은 Notificationo이 있을경우, 해당 사용자의 HomeFragment Notification 아이콘에 표시 추가
...
6. 안읽은 메세지가 있을경우, 해당 사용자의 ChatFragment 아이콘에 표시 추가
