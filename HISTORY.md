...
3. AuctionRoomActivitiy에 수량 탭 추가 >> 품목 밑에 조그맣게

4. 입찰 버튼 누르면 얼마로 입찰될 예정인지 혼란스로울 수 있으니 예상 입찰 가격 추가 ex) 현재 50,000원이면 예상 입찰가 50,500원 이런식으로

5. Profile Fragment에 판매 내역, 찜 내역, 최근 본 목록, 입찰 내역도 AuctionAdapter 연결해주고 데이터 표시 방법 동일하게 함

6. 구매 희망 내역 수정   
    - 게시글 삭제 버튼 맨아래 > 오른쪽 위에 아이콘버튼화
    - 구매 희망 글 >> 최신 순으로 reverse 정렬
    - 상세 설명 박스화
    - 검색 버튼 활성화
    - 댓글 삭제 기능 추가(오른쪽 위에 아이콘 버튼 추가)
    - 내 구매 희망 건은 내가 댓글 못달게 수정

7. 구매 희망 글 작성시 homefragment가 아닌 pastfragment로 이동하도록 수정
...
1. ProfileFragment에 "구매 희망 내역 " 구현


2024-10-16

<한 일 >
1. ChatActivity에서 나가기 기능 구현

2. Createroomactivity에서 입력필드 중 하나라도 입력되지 않으면 완료 버튼 비활성화

3. 검색창에 커서 활성화(포커스)되면 검색어를 입력하세요 힌트 비활성화
...
0. hyeonlee717@gmail.com 로그아웃 -> 로비에서 회원가입 -> 회원가입에서 다른 이메일로 인증 후 회원가입 버튼 활성화 후 누름 -> 로비로 가지더니 바로 이전 hyeonlee717@gmail.com으로 다시 로그인됨 -> 다시 로그아웃하고 새로운 아이디 로그인 하니까 되긴 됨
...
7. PostFragment UI 수정

1. 경매 종료시에, 판매자가 판매내역에서 AuctionRoomActivity를 들어갔을 때 detail 하단에 최고 입찰자 3명과 채팅 할 수 있는 기능 구현(gone)

1. HomeFragement 우측 상단에 알림 기능 구현( 내 판매내역에 입찰 들어왔을때, 내 입찰이 갱신되었을때(다른사람한테), 내 찜목록이 1시간이하로 남았을때, 
 내 구매희망 글에 댓글이 달렸을 때 등등)

2. 중복 로그인시 프로필 메뉴에 다른 기기에 접속한 계정 로그아웃되게 하는 기능 추가 "다른 기기에서 로그인 해제가 요청되었습니다."

3. 안읽은 메세지가 있을경우, 해당 사용자의 ChatFragment 아이콘에 표시 추가

4. 내가 만든 경매에 입찰이 없을 경우, 판매자가 해당 경매 삭제 할수 기능 추가(실수로 생성했을수 있음)

5. 판매자가 자신의 AuctionRoomActivity에 들어갔을떄 현재는 bid, chat 버튼이 비활성화  되어있음

    --> 안보이게 수정, 그 자리에 Delete 버튼을 추가

6. AuctionRoomActivity --> 참가자 수 삭제, 최고가, 입찰가 한글 삭제 , "-" 추가, 입찰 버튼의 위치를 입찰 단위 자리로 옮겨 보기.

3. 희망 거래 장소, gps 기반 위치?