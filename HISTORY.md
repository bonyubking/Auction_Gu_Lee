...
1. 알림 전체 읽기 기능 구현 , 채팅 전체 읽기 기능 구현

2. 안읽은 메세지가 있을경우, 해당 사용자의 ChatFragment 아이콘에 안읽은 메시지 개수 표시 추가

3. AuctionRoomActivity에서 입찰 버튼 클릭시 정말 입찰하시겠냐는 팝업 알림 출력
...
5. 로그인 된 계정을 다른 기기에서 로그인 시도할 때, 이메일 인증(파이어베이스)으로 다른 기기에서 로그인 해제하고 로그인. users에 device항목 추가하여 기기 고유번호 등록 후 로그인 관리?
